...
const AppointmentConfirmation = () => {
  ...
  useEffect(() => {
    const fetchAppointment = async () => {
      try {
        const response = await api.get(`/api/agenda/appointments/token/${token}`);
  ...
  const handleAction = async (action) => {
    setProcessing(true);
    try {
      await api.post(`/api/agenda/appointments/token/${token}`, {
        action,
      });
      navigate(`/appointment-result?status=${action}`);
    } catch (err) {
    ...
    <div className='min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8'>
      <div className='bg-white p-8 rounded-lg shadow-md max-w-md w-full'>
        <div className='text-center mb-8'>
          <h2 className='text-3xl font-bold text-gray-900 mb-2'>Confirmación de Cita</h2>
          <p className='text-gray-600'>Por favor, confirma o cancela tu cita</p>
        </div>
        ...
        <div className='flex flex-col space-y-4'>
          ...
          >
            {processing ? "Procesando..." : "Confirmar Cita"}
          </button>
          <button
